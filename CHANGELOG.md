...
- Rebuilding an index from identical sources produces the same chunks and rankings. Tree-sitter parsing is budgeted in parser operations (500,000 per file) instead of 100 ms of CPU time, so large files such as ivygrep's own `src/search.rs` no longer switch between structural and fallback chunks with machine load; a CPU allowance per operation still stops pathological inputs. Lexical, path, literal, and Boolean candidate pools collect past equal BM25 scores until the tie band at the cutoff is complete, and order ties by path, span, and chunk key instead of Tantivy document addresses, which depend on indexing threads and segment merges. Three codetrans-dl builds with default, 1, and 8 index threads previously disagreed on 126 to 144 of 180 top-20 lists per pair; they now agree on all of them.
