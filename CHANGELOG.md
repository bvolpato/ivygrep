...
- Neural search results now depend on how good the embedding model is, and the default neural profile is `potion-code-16m-v2` instead of `static-retrieval-v1`; an unset or unknown `IVYGREP_MODEL_PROFILE` selects it (see the upgrade notes). Until now the neural tier was one reciprocal-rank vote of weight 1.0 among lexical, literal, path, and symbol votes worth several times more, and only files among the 50 nearest chunks got it, so a better model changed almost nothing: on the public benchmarks `potion-code-v2` beat `static-retrieval-v1` by .003 nDCG@10 in default mode although its own ranking is far better. A late dense-fusion stage now runs after the ranking heuristics and before result filtering and the learned reranker. Search scores every candidate that lexical, literal, path, or symbol search found against the neural query vector, then reorders files by two signals. A file whose best cosine beats the runner-up file by a ratio of 1.05 moves to the first position. Profiles with a measured calibration (`DenseFusionCalibration` in `src/embedding.rs`; `potion-code-16m-v2` uses 0.25 for pasted source and for multi-line or long prose) also add a standing reciprocal-rank vote, and search consults them for every query whose route allows neural retrieval instead of only when lexical confidence is low. `static-retrieval-v1` and unmeasured profiles have no standing vote and keep the lexical-confidence gate. Only files that direct search found, that pass the result filter's authority test, and that have the most authoritative path role among the direct candidates take part, and they are reordered among the positions they already hold, so cosine alone never promotes a file and documentation, tests, and support files never move relative to the implementation. Short natural-language queries skip the stage. Scores are reassigned by position, so the adaptive score filter and the learned reranker see the same distribution as before. All figures are measured on top of the long-query lexical vote in this release, against `static-retrieval-v1` as the default without the new stage. On 923 held-out public queries (the public-core halves the reranker was not fit on, four `sota-challenge` samples, and CodeSearchNet Python and Java), default-mode nDCG@10 rises by .011 (102 queries up, 68 down; CodeSearchNet-java .677 to .727, codefeedback-st .779 to .803, codetrans-contest .588 to .609): .008 from the new stage with the same model (.009 with `--force-neural`) and .003 from the model. On the file-localization benchmark default mode goes from Acc@1 .367 to .400, Acc@5 .767 to .800, Acc@10 .767 to .833, and MRR .524 to .551. On MemoryQuest notes retrieval nDCG@10 goes from .435 to .455 and recall@20 from .753 to .764. The self-repository relevance gate ranks every query the same in every mode, and lexical, hash, and hybrid results without neural vectors are identical. With `IVYGREP_MODEL_PROFILE=static-retrieval-v1` no default-mode query changes. The new model is a quarter of the download, index size is unchanged (256 dimensions, f16), both models embed 25,460 chunks in 6 seconds, and peak memory during that enhancement is 140 MiB against 179 MiB. CI and release acceptance now cache and validate the default profile (`Model2Vec token mean via Rust`), and benchmark results recorded without `IVYGREP_MODEL_PROFILE` are attributed to `potion-code-16m-v2`.
- The static musl builds for Linux, which `install.sh` and the Homebrew formula install, use jemalloc as their allocator on 64-bit targets. musl's own allocator takes one lock for every thread, and ivygrep allocates from many threads at once: a full index plus enhancement of this repository took 30 s with the musl archive against 5 s with a glibc build, and a daemon under 64 `ig --mcp` sessions served 34 calls per second against 159, with a median hybrid search of 1.6 s against 0.3 s. With jemalloc the same index takes 6 s and the daemon serves 65 calls per second at a median of 0.7 s. The daemon uses more memory under that load (425 MiB instead of 236 MiB, a glibc build 688 MiB) and about the same when idle; an idle `ig --mcp` session goes from 1.2 to 1.8 MiB, still 3 threads. jemalloc fixes its page size at build time and aborts on a kernel with larger pages, and aarch64 kernels use 4, 16, or 64 KiB, so the aarch64 archive is built for 64 KiB (`.cargo/config.toml`). `ig hardware` now reports the allocator and, for jemalloc, that page size (`allocator` in `--json`), and the release and E2E workflows assert it for the musl archives, because emulation with 4 KiB pages cannot. glibc, macOS, and Windows builds keep their system allocator and report `system`.
...
- Multi-paragraph prompts, pasted issue text, and questions with a blank line are ranked as prose instead of pasted source. In v1.2.16 every multi-line query left the signature field out of lexical scoring, stopped turning `owner.member` mentions into exact-symbol lookups, and kept hash votes on direct candidates. Multi-line input is now pasted source only when it reads as code: at least half of its words sit on code lines (ending in `;`, `{` or `}`, indented, or with as many code-shaped tokens such as `=`, `snake_case`, `owner.member` or `call(arg)` as words) or are code-shaped themselves. Multi-line prose scores signature matches like body matches instead of at 5x, looks up `owner.member` mentions, and gives hash matches on direct candidates no fusion vote. Pasted source and multi-line pasted error output keep v1.2.16's rules, and an explicit `signature:` clause keeps its 5x boost in any query. On 24 multi-paragraph agent tasks, issue reports, and questions about ivygrep's own source, MRR on the held-out half rises from .708 to .750 in foreground mode and from .757 to .808 in neural mode. On public-core codefeedback-st, lexical nDCG@10 rises from .7189 to .7227 and recall@20 from .8182 to .8283. codetrans-contest, codetrans-dl, and cosqa lexical results are unchanged, and the self-repository gate gives identical results. Scoring prose signature matches at 5x instead would have dropped codefeedback-st lexical nDCG@10 to .6301.
- Multi-line classification from the previous entry reads more pastes and prompts correctly. Pasted code led by import statements (`import numpy as np`, `from … import …`, and `require`, `use` or `package` with one argument) is pasted source again: a snippet with two import lines and three `plt.` calls read as prose, so `plot`, `savefig`, and `show` became exact-symbol lookups and one-line definitions of those names outranked the code that contains the snippet. Indented paragraphs and hard-wrapped list items under prose, Markdown tables, rules, and heading marks, `？`, `。`, curly quotes, and `…`, issue references, versions, and sizes such as `#375`, `v1.2.16`, and `2.3GB`, and `e.g.` and `and/or` no longer make prose read as pasted source. In multi-line prose, `owner.member` mentions become exact-symbol lookups only on prose lines and only when written as a call, in backticks, or with a camelCase member, so `go.sum` or `vs.Code` no longer look up `sum` or `Code`. Explicit `signature:` range and set clauses score the same on every query shape; multi-line prose had boosted them 5x. The snippet fixture ranks the file that contains the snippet first again, the #394 question with its second paragraph indented ranks the definition first, and a question mentioning `go.sum` ranks the checksum code above a `def sum` file. codetrans-contest, codetrans-dl, and codefeedback-st classify every multi-line query the same as before. 2 of 99 multi-line questions in a stackoverflow-qa sample, a quoted hard-wrapped problem statement and a SQL question with ASCII result tables, now read as prose. public-core lexical results are identical, and the self-repository gate and the 24-question prose benchmark rank every query the same.
- Repositories made of `.sql` files answer natural-language queries again. `.sql` was classified as a data file, whose authority sits below every recommendation floor and cannot relax it, so a candidate list with only SQL files was filtered to an empty result. On a 100-query text-to-SQL sample over 5,000 `.sql` documents, 76 queries returned nothing; 1 does now, and nDCG@10 rises from .217 to .857. SQL under fixture and generated paths keeps its lower authority.
- The daemon releases the watcher of a workspace whose directory no longer exists, such as a deleted Git worktree. It kept the watch backend (on Linux one inotify instance, one thread, and three descriptors), a heartbeat, and a retry loop that logged `watch update failed` every 30 seconds for the rest of the daemon's life, and the supervisor retried every dead root every 15 minutes. After 500 agent worktrees were created, searched through MCP, and removed, the daemon held 555 threads (44 before), 1,522 descriptors (18), 501 inotify instances (1; the usual Linux limit is 128 per user), and 790 MiB of anonymous memory (104), and none of it came back. It now ends the same run with 44 threads, 21 descriptors, and one inotify instance. Anonymous memory still moved 35 MiB over the 500 worktrees, which is freed memory held in glibc malloc arenas, not state per worktree: with `MALLOC_ARENA_MAX=2` the daemon stayed between 119.5 and 120.6 MiB from the 100th to the 300th worktree. The watch worker releases the watcher when an update fails because the root is gone, and the supervisor pass covers roots that vanish without an event. `ig --status` still says why such a workspace is not watched: the watcher job is recorded once as failed with `workspace directory no longer exists`, without a retry or a backoff entry. `watch_enabled` stays set, so the same path checked out again is watched by the next request or supervisor pass, which clears the record. A root that exists but cannot be watched keeps its retries and backoff.
- Long natural-language prompts rank by their lexical evidence. A plain BM25 ranking beat ivygrep's lexical mode by .06 to .19 nDCG@10 on prompts and questions, because a BM25 margin barely reached the fused score (rank votes are nearly flat and the score term is logarithmic) and structural boosts then decided the order. For queries of 13 or more terms, pasted source included, lexical candidates now also vote with their share of the best BM25 score. One-line prompts of that length score signature matches like body matches, as multi-line prose already does, and a sentence-case word such as "Create" is no longer inferred as a symbol name. On held-out halves of seven public tasks, lexical-mode nDCG@10 rises by .041 on average (codefeedback-st .690 to .781, stackoverflow-qa .542 to .632, codetrans-contest .543 to .598); CodeSearchNet, never used for tuning, goes from .920 to .940 for Python and .601 to .664 for Java. The self-repository gate ranks every query the same, and the 30-task file-localization benchmark is neutral (Acc@5 .733 to .767, MRR .514 to .524, Acc@1 .400 to .367). A 24-question prose benchmark on ivygrep's own source is unchanged in its multi-line form; the same wording on one line moves from .793 to .759 nDCG@10, which is not significant. The vote reorders results but never shortens the list the score filter returns: files that clear the score filter only without the vote are appended after the results it kept, one chunk per file, which keeps recall@20 on the MemoryQuest notes benchmark (lexical mode +.003, default mode +.004 on its held-out half; without that rule -.017 and -.008) while nDCG@10 there rises by .015 in default mode. Shorter queries are unchanged.
- `ig --mcp` no longer starts one idle Tokio worker thread per core. The MCP server is blocking stdio code and runs each daemon request on a short-lived runtime of its own, so it now uses a current-thread runtime: an idle session has 3 threads instead of 19 on a 16-core machine.
- MCP context packs (`output=context_pack`) are built by the daemon, like searches. The MCP process built them itself and then kept the query model, a preview cache of up to 64 MiB, and the index thread pools for the rest of the session: 74 MiB of anonymous memory and 44 threads per session after the first pack, against 2 MiB and 3 threads for a session that only searches, and without the daemon's CPU permits or workspace leases. The daemon now pays for the model and the cache once. `notifications/cancelled` and a client that disconnects stop the build through the request id, as for searches, and the pack is byte-identical to the in-process one, which remains the fallback when no daemon answers. The daemon protocol is now version 9, and a daemon keeps serving version 8 clients, so MCP sessions that were running before an upgrade keep using the new daemon instead of restarting it on every call.
- A Git worktree or clone nested inside a Git workspace is no longer indexed as part of it. Agent worktrees under `<repo>/.claude/worktrees/` sit inside the base repository, and `.claude` is usually untracked rather than ignored, so every live worktree put a full copy of the repository into the base index: three worktrees took a base from 60 files, 4,024 chunks, and 4.4 MB to 240 files, 16,096 chunks, and 17.6 MB, base searches returned every file four times, and the base reindexed a whole checkout whenever a worktree was created or removed. The file walker, the path matcher, and the watcher event filter now skip a nested directory whose `.git` entry marks a linked worktree or a clone, including a clone made with `git clone --separate-git-dir` and worktrees created after the base was indexed, and the same three worktrees leave the base unchanged. Existing base indexes drop the copies on their next full walk. A directory that is already indexed and then becomes a repository (`git init`, or a clone into it) leaves the parent index when its `.git` entry appears, and its files return when the entry is removed; an agent worktree, which arrives with its directory, costs the base one lookup and no scan. Submodules stay in the parent (a path in `.gitmodules`, or a Git directory below the parent's `.git/modules`), and a workspace root that is not a Git checkout still indexes the clones below it.
- The Web UI server closes each connection gracefully after its response: it shuts down its sending side, discards unread input until the client closes too (for at most 1 second and 1 MiB), and only then drops the socket. It dropped the socket right after writing. A socket dropped with unread input, such as a request body or bytes sent after the headers, is reset instead of closed, so a client that read to the end of the connection got `connection reset` after the response, and on Windows a reset also discards response bytes the client has not read yet. Shutting down first also ends the response when another process still holds a handle to the connection, where dropping the socket sent nothing.
- Indexes of workspaces that no longer exist are removed. Nothing removed them before, so every deleted worktree left its index directory behind and stayed in `ig --status` and `ig_status`: 500 removed worktrees left 501 index directories. The daemon now removes an index once its root has been missing, not merely unreadable, for `IVYGREP_INDEX_GC_GRACE_SECS` (seven days by default, `0` disables it), and the overlay of a worktree that `git worktree list` no longer reports after ten minutes. It never removes an index whose root exists, that a live worktree overlay still reads as its base, or whose index or enhancement lock or job is in use. A pass takes the daemon's exclusive mutation lease before it touches an index, like `ig --rm`, so a search that still reads it finishes first, and it checks the root again once it holds the lease and the locks, so a checkout that came back while the pass was on its way keeps its index. `ig --gc` runs one pass by hand and reports what it removed and what is waiting; a running daemon makes that pass, since it holds readers and watchers of the indexes that go, and the CLI makes it only when no daemon runs.
- A running daemon rotates `daemon.log` at 10 MiB on Unix. The log was only rotated when a client spawned a daemon, so a daemon that stayed up for days grew it without bound; with the watcher retry loops above it grew 5 MB in 30 minutes. The daemon checks once a minute and redirects its own stdout and stderr to the fresh file, and leaves output that goes to a terminal or a service manager alone.
- On Windows, child processes no longer inherit the daemon's sockets. The locked mio 1.2.0 created Windows sockets with plain `socket()`, which leaves the handle inheritable, so every `git`, editor, and enhancement worker process the daemon started got a handle to the loopback IPC listener and to the connections open at that moment. A connection held that way stays open after the daemon closes it, and a listener held by a long-running worker keeps its port bound after the daemon exits. mio 1.2.3 creates sockets with `WSA_FLAG_NO_HANDLE_INHERIT`. Only the lock entry for mio changes.
- On Linux with glibc an idle daemon returns freed memory to the OS. glibc keeps freed memory in one malloc arena per thread, and after a burst from many sessions nearly all of the daemon's anonymous memory was free space inside about a hundred arenas that it kept for as long as it ran: 668 MiB after 32 sessions stopped calling. The daemon now calls `malloc_trim` once after 60 to 90 seconds without a request or a watch update, and it never trims while requests arrive: in the same workload it went from 697 MiB to 228 MiB 80 seconds after the last request. Capping the arenas with `MALLOC_ARENA_MAX=4` instead cost 16% of the throughput and doubled the median search latency. This concerns glibc builds (from source, and the CUDA archive); the static musl archives compile it out and do not need it, because musl's allocator gives memory back as it is freed.
- Background enhancement runs at most `IVYGREP_ENHANCE_MAX_WORKERS` worker processes at once (default 2) per lane, hash and neural, across all workspaces of the app home. The daemon started a worker for every workspace that a search or a watch update found stale, all at once: 20 workers holding 1,278 MiB when 20 workspaces were edited together, 1.2 GiB for six minutes when the load guard paused them in the middle of neural runs. Requests now wait in a daemon queue, the workspace that was searched or edited last goes first, and a workspace whose directory is gone leaves the queue. A worker takes its place through a lock file under `$IVYGREP_HOME/enhancement-slots/` before it loads a model or opens a store, so the limit also holds for workers that `ig` or an MCP session starts, a dead worker frees its place, and a worker that the memory, battery, or load guard holds back waits with nothing loaded instead of keeping a model resident. The guards follow the pass: a neural worker builds its hash vectors under the hash tier's guards, so they are still built on battery, and gives up its place while the neural tier's guards keep it from loading the model. Hash workers have their own places, so a fresh worktree gets hash vectors without waiting behind neural runs. `--wait-for-enhancement` prints when its worker is queued. In the same storm four workers ran at the peak and held 237 MiB; the workspace being searched had neural vectors after 12 s as before, a fresh worktree had hash vectors after 2 s as before, and the last of the 20 workspaces was done after 54 s instead of 18 s. Raise the limit on a host with cores to spare.
- `scripts/soak_mcp_sessions.py` soaks many `ig --mcp` sessions against one shared daemon, which the daemon soak never did: a startup stampede with no daemon running, sessions ended by SIGKILL, closed stdin, or closed stdout (half of them mid-request), concurrent sessions issuing mixed hybrid, literal, regex, symbol, context-pack, and status calls across several workspaces, and agent-worktree churn under `<repo>/.claude/worktrees`, plus optional idle-cost and background-storm phases. It samples anonymous RSS, threads, descriptors, and inotify use of the daemon and of every session from `/proc` and fails on growth past the daemon soak's budgets, a session that outlives its client, a second daemon, watchers or index directories left by deleted worktrees, and base results that come from a nested worktree. The daemon and the sessions run with `MALLOC_ARENA_MAX=2` by default, so that anonymous RSS follows memory in use instead of memory that about a hundred per-thread malloc arenas happen to hold; `--malloc-arenas default` measures the production configuration. `--mode short` takes about two and a half minutes and runs in the performance workflow after the daemon soak; `--mode long` is the multi-hour run behind the [measured results](docs/benchmarks/daemon-soak.md#many-mcp-sessions).
- A prompt or paste with an uppercase `AND`, `OR`, or `NOT` no longer fails when it is not a valid Boolean expression. Those standalone words are Boolean operators, and a request that did not parse failed with `invalid or unsupported Boolean query`. That error teaches the syntax to someone who typed `foo AND`, but pasted issue text, a Stack Overflow question with SQL, or a task description with an emphasized `NOT` is not an attempt at a Boolean expression, and an agent calling `ig_search` got an error instead of results. A query that spans several lines or has 13 or more terms, the cut that already separates prompts and pastes from one-line lookups for signature scoring, and that does not parse now takes the ordinary search path with the operator words read as text. The results carry a warning that the operators were not applied: on stderr for the CLI, without the `partial search` label, and in `warnings` for MCP, the daemon, and the Web UI, once per search. A one-line query under 13 terms, such as `foo AND` or `(a OR b`, keeps the error, and a query that parses is a Boolean request as before, so results change only for queries that used to fail. The daemon does not cache results that carry the warning, because its query cache stores hits only. Context packs use the same search and no longer fail on such a task, but have no field for the warning. On the 600-query `sota-challenge` profile, 6 queries failed this way (4 stackoverflow-qa, 1 apps, 1 codefeedback-mt) and the first one stopped the evaluator; all 6 return results now, and in lexical mode, whose rankings repeat exactly from run to run, the other 294 queries of those three tasks return the same ranked lists as before. None of those 600 queries and none of the 1,000 `public-core` queries parses as a Boolean expression.
- A query with a standalone `-` or `+` followed by a standalone `*` no longer crashes the search. Tantivy's query grammar panics on that input instead of rejecting it (`Exist query without a field isn't allowed`), and a Markdown list in a pasted question is enough: `ig` exited with status 101, and a daemon or MCP search failed. Two of 19,931 Stack Overflow questions in the public retrieval pool triggered it. Such a query is now reported to the search as a syntax error without calling the grammar, so it takes the plain term search that any text the grammar rejects already takes, and a panic from input not found yet is caught the same way.
- Reranker capture records (`IVYGREP_RERANKER_CAPTURE=1`) name the query that was requested. For pasted error output the record named the text ranking signals use, with runtime values such as numbers and paths removed, so `eval_code_retrieval.py --capture-reranker` rejected it with `native capture query does not match the requested query` and stopped the whole dataset. On main that stops the `reranker-fit` collection at codetrans-contest (queries q207 and q296) and rejects 27 of 200 codefeedback-mt and 8 of 200 stackoverflow-qa queries of `reranker-train`. Captured candidates and feature vectors are unchanged, and so is search output.
- The learned reranker no longer penalizes code files against docs, templates, and scripts. Its model gave `primary_source` (the file has a code extension and is not under a support directory) a weight of -1.97, and smaller weights to `query_path_coverage` (-0.57) and `path_term_f1` (+0.14). The model was fit on public benchmark exports, which store every document at `documents/<position>.<extension>`: three of the four fit corpora have a single extension, so `primary_source` cannot differ inside a candidate list there, and in the fourth, codefeedback-st (50 of the 481 fit queries), it is the language tag of the answer. A fit on today's captures of the same queries gives that weight the opposite sign. In a repository the feature is 1 for code and 0 for an issue template, so pasted issue text and other multi-line queries, the ones the learned stage reorders, ranked prose above the code they describe. The three weights are now zero, the model file records them and their fitted values under `fixed_zero_features`, and `scripts/train_public_reranker.py` reads path features as zero for corpora with such paths, in the fit and in the validation and evaluation that judge it, so a later fit cannot learn them again. Every other weight is unchanged. The model file's `evaluation` record is the trainer's evaluation of the shipped weights (`--reevaluate`) on captures taken at main: nDCG@10 +3.50% and MRR@10 +4.43% over the deterministic order on 848 held-out candidate lists, short of the 5% offline gate, and exactly the same for the fitted weights. The records of the original fit, which showed +11.25% on a ranking stack that no longer exists, stay in the file as `original_fit` under the checksum of the fitted weights. Every metrics record names the weights it was computed for, and `render_public_reranker.py` refuses a model whose record names other weights. On the file-localization benchmark (30 issues, default route with neural vectors) Acc@1 goes from 0.400 to 0.467 and MRR from 0.551 to 0.611, in hash mode from 0.367 to 0.433 and from 0.524 to 0.579; the deterministic ranker scores 0.533 and 0.645. The public benchmarks cannot see the change, which is why they never caught the weight: on the 520 `reranker-eval` queries nDCG@10 moves by -0.0001 (3 queries up, 5 down) and on the 600 `sota-challenge` queries by +0.0008 (2 up, 2 down), and the self-repository relevance gate gives the same result for every query.

### Documentation

- The README and the `ig_search` MCP tool schema say that standalone uppercase `AND`, `OR`, and `NOT` are Boolean operators, and that lowercase words, backticks, or quotes search them as text. Neither mentioned it, so a bug report pasted as a query, such as "the watcher does NOT reload settings", silently excluded every result containing "reload", the word after `NOT`.
...
- The public code-retrieval report, the embedding model bake-off, the self-repository snapshot, and the benchmark dashboard are measured again at main commit `2f4208d`, with `potion-code-16m-v2` as the default model and the ranking changes made since v1.2.16. They had still shown `static-retrieval-v1`: the v1.2.16 release matrix, and a v0.10.1 bake-off that named `static-retrieval-v1` the selected default. The bake-off now records `potion-code-16m-v2` as selected and `static-retrieval-v1` as the previous default, each with a complete screening matrix from the same binary. main still reports v1.2.16, so the reports name the commit. The matrix ran on a 16-CPU Linux host instead of the 4-CPU GitHub runner of the release matrix: quality is comparable with v1.2.16, latency, indexing time, and memory are not. The held-out challenge, MemoryQuest, and learned-reranker reports keep their earlier measurements.
...
- The embedding bake-off renderer takes the selected default from `benchmarks/public/model_candidates.json`. It had `static-retrieval-v1` hard-coded, in the `selection` field and in two sentences of the page, so rendering new evidence would still have published the old model as selected. It now fails unless exactly one candidate is `selected-default` and that candidate has a complete screening matrix, and a test checks the published report against the candidates file. The report records the renderer's SHA-256 instead of a checkout revision; the revision in the previous report is not in the repository.
- Public benchmark pages name the measured commit next to the version, as in `Measured with v1.2.16 at commit 2f4208d79493`. main reports the last release's version until the next bump, so the version alone credited that release with a later build.
- The public relevance gate asks more of two datasets: `codefeedback-st` needs nDCG@10 0.70 (was 0.62) and recall@20 0.81 (was 0.72), and `codetrans-contest` needs nDCG@10 0.40 (was 0.365). A floor applies to every required mode, so the weakest mode sets it. Since v1.2.16 the weakest mode rose from 0.7117 to 0.7950 and from 0.8182 to 0.9091 on `codefeedback-st`, and from 0.5578 to 0.5959 on `codetrans-contest`. Each new floor keeps the distance its old floor had to the v1.2.16 value. The `codetrans-dl` and `cosqa` floors stay: the weakest `codetrans-dl` mode did not move, and the weakest `cosqa` mode gained 0.0043 nDCG@10 and seven of 500 queries in recall@20, too little to carry a tighter floor.
- The Web UI unit tests create their loopback sockets through `std`, as the daemon does for its listener. `web::tests::half_closed_search_request_still_receives_response` began failing on Windows with `ConnectionReset` (10054) once another test in the same process ran `git` at the same time: a socket Tokio creates on Windows is inheritable with the locked mio 1.2.0, so each `git` process got a handle to the accepted connection and the server's close no longer ended it. New tests cover a request with unread bytes, with and without a half-close, a connection that a second handle keeps open, and the time and byte bounds of the graceful close. The first two fail on Linux without the server fix.
...
- Hash ANN candidates are near neighbors again on large workspaces. The hash tier built its HNSW graph with connectivity 2 and add expansion 8, which returned almost random neighbors past 50,000 vectors: recall@10 against exact search was .01-.05 on held-out code chunks. It now uses connectivity 8 and add expansion 32, reaching recall@10 .89-.91 at 50,000 and 200,000 vectors. On CodeSearchNet-python (281,109 chunks, 300 queries), hash-mode nDCG@10 rose from .834 to .847 with no per-query losses. Background hash enhancement takes longer (CodeSearchNet-python: 18 s to 119 s; a 683K-chunk monorepo: 52 s to 133 s), and hash stores grow about 7%. Existing hash stores keep the old graph until the index is rebuilt, for example with `ig --add <path> --force`. The guarded hot hash-vector benchmark is renamed `hash_vector_search_in_50k_distinct_hot`, so its paired baseline starts from the new graph.
...
Tagged but not published as a GitHub release. These changes first shipped in 1.2.14.
...
- Live search previews read regular files within the selected workspace and fall back to indexed text when the current source path is unavailable.
- Git-history context reads pin the working directory on Unix. Historical-only seeds are skipped on platforms without safe subprocess anchoring; current-file and indexed evidence remain available.
- Index format v26 rebuilds legacy indexed payloads using contained source reads, including unchanged files and their derived vectors. Direct indexed-source APIs reject incompatible local or inherited base formats until rebuilt.
...
- Failed main-store validation triggers a complete staged rebuild instead of replaying an incremental delta into empty stores. Unchanged files remain searchable after recovery, and interrupted recovery preserves the previous stores and workspace settings.
...
- Symbol references include function values and callbacks, while caller results require calls. Syntax verification handles whitespace, multiline and generic calls without reporting declaration names, comments, or string contents; bounded searches refill rejected candidates up to the existing lexical ceiling instead of letting definitions consume a small result limit.
- Broad include and exclude globs retain matching lexical, literal, and path-recall candidates after the 10,000-path filter threshold. Overflow filters run inside the bounded candidate collector before rejected paths can displace matching results.
...
- The CLI no longer restarts the daemon when a workspace's watcher looks offline. That restart tore down every other workspace's watcher and in-flight work, and repeated on every query whenever the watcher could not come back: an index rebuild wipes the job ledger (`job.json`) the liveness check reads, so after the v22 format bump every watched workspace would have looked offline forever; watcher registration also races the daemon's startup and fails outright under inotify limits. The watcher heartbeat now re-creates its ledger record when the ledger was wiped, the daemon re-registers missing watchers at startup and every 30 seconds with exponential backoff (30 s to 15 min) after failures, the CLI asks for that via a new `EnsureWatcher` request instead of `Restart`, and `ig --status` shows the last watcher start error next to "watcher offline". Daemon protocol version 8; older daemons restart on the next request.
