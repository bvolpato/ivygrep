...
- **Privacy and neural-backend documentation now matches runtime (#75).** Search and embedding inputs stay local; neural mode does fetch model artifacts on first use through `hf-hub`. Current releases use Candle CPU inference, with macOS builds enabling Accelerate-backed CPU math; they do not claim Metal, CoreML, or CUDA execution without an implemented backend.
