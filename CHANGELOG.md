...
- The challenge retrieval report is refreshed at main commit `e9119a4`, after the Boolean-prose fallback and query-grammar panic guard. All 600 queries run without exclusions in hash, hybrid, blended, and neural modes, with three repetitions each. The report discloses which samples were used for dense-fusion calibration and that local glibc measurements are not a controlled timing comparison with the previous report or shipped musl archives. The dashboard labels the learned-reranker report as historical v0.10.1 evidence; shipped weights and quality floors are unchanged after removing synthetic-path weights caused a significant loss on a separate codefeedback-st holdout.
